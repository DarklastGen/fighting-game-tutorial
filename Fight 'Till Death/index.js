// JavaScript Document

//Javascript Game Manager
const canvas = document.querySelector('canvas');
const c = canvas.getContext('2d');

canvas.width = 1024;
canvas.height = 576;

c.fillRect(0, 0, canvas.width, canvas.height);

const gravity = 0.67;

//Background
const background = new Sprite({
	position: {
		x: 0,
		y: 0
	},
	imageSrc: './img/background.png'
})

//Shop
const shop = new Sprite({
	position: {
		x: 575,
		y: 177
	},
	imageSrc: './img/shop.png',
	scale: 2.65,
	framesMax: 6
})

//Player 1
const player = new Fighter({
	position:{
		x: 0,
		y: 0
	},
	velocity:{
		x: 0,
		y: 0
	},
	offset: {
		x: 0,
		y: 0
	},
	imageSrc: './img/samuraiMack/Sprites/Idle.png',
	scale: 2.5,
	framesMax: 8,
	offset: {
		x: 215,
		y: 155
	},
	sprites: {
		idle: {
			imageSrc: './img/samuraiMack/Sprites/Idle.png',
			framesMax: 8
		},
		run: {
			imageSrc: './img/samuraiMack/Sprites/Run.png',
			framesMax: 8
		},
		jump: {
			imageSrc: './img/samuraiMack/Sprites/Jump.png',
			framesMax: 2
		},
		fall: {
			imageSrc: './img/samuraiMack/Sprites/Fall.png',
			framesMax: 2
		},
		attack1: {
			imageSrc: './img/samuraiMack/Sprites/Attack1.png',
			framesMax: 6
		},
		takeHit: {
			imageSrc: './img/samuraiMack/Sprites/Take Hit - white silhouette.png',
			framesMax: 4
		},
		death: {
			imageSrc: './img/samuraiMack/Sprites/Death.png',
			framesMax: 6
		}
	},
	attackBox: {
		offset: {
			x: 100,
			y: 50,
		},
		width: 150,
		height: 50
	}
})

//Player 2
const enemy = new Fighter({
	position:{
		x: 400,
		y: 100
	},
	velocity:{
		x: 0,
		y: 0
	},
	offset: {
		x: -50,
		y: 0
	},
	color: 'blue',
	imageSrc: './img/kenji/Sprites/Idle.png',
	scale: 2.5,
	framesMax: 4,
	offset: {
		x: 215,
		y: 165
	},
	sprites: {
		idle: {
			imageSrc: './img/kenji/Sprites/Idle.png',
			framesMax: 4
		},
		run: {
			imageSrc: './img/kenji/Sprites/Run.png',
			framesMax: 8
		},
		jump: {
			imageSrc: './img/kenji/Sprites/Jump.png',
			framesMax: 2
		},
		fall: {
			imageSrc: './img/kenji/Sprites/Fall.png',
			framesMax: 2
		},
		attack1: {
			imageSrc: './img/kenji/Sprites/Attack1.png',
			framesMax: 4
		},
		takeHit: {
			imageSrc: './img/kenji/Sprites/Take hit.png',
			framesMax: 3
		},
		death: {
			imageSrc: './img/kenji/Sprites/Death.png',
			framesMax: 7
		}
	},
	attackBox: {
		offset: {
			x: -150,
			y: 50
		},
		width: 150,
		height: 50
	}
})

//Check properties on Web Console || Monitoring Purposes
console.log(player);

const keys = {
	a: {
		pressed: false
	},
	d: {
		pressed: false
	},
	ArrowRight: {
		pressed: false
	},
	ArrowLeft: {
		pressed: false
	}
}

//The timer
decreaseTimer();

//No Animation No Life
function animate() {
	window.requestAnimationFrame(animate);
	c.fillStyle = 'black';
	c.fillRect(0, 0, canvas.width, canvas.height);
	background.update();
	shop.update();
	c.fillStyle = 'rgba(255, 255, 255, 0.15)'
	c.fillRect(0, 0, canvas.width, canvas.height)
	player.update();
	enemy.update();
	
	//Making sure not to drag
	player.velocity.x = 0;
	enemy.velocity.x = 0;
	
	//Player Movement
	if(keys.a.pressed && player.lastKey === 'a') {
		player.velocity.x = -4.76;
		player.switchSprite('run');
	}else if(keys.d.pressed && player.lastKey === 'd') {
		player.velocity.x = 4.76;
		player.switchSprite('run');
	}else {
		player.switchSprite('idle');
	}

	//Jumping Animation
	if(player.velocity.y < 0) {
		player.switchSprite('jump');
	} else if (player.velocity.y > 0) {
		player.switchSprite('fall');
	}

	//Enemy Movement
	if(keys.ArrowLeft.pressed && enemy.lastKey === 'ArrowLeft') {
		enemy.velocity.x = -4.76;
		enemy.switchSprite('run');
	}else if(keys.ArrowRight.pressed && enemy.lastKey === 'ArrowRight') {
		enemy.velocity.x = 4.76;
		enemy.switchSprite('run');
	} else{
		enemy.switchSprite('idle');
	}

	//Jumping Animation
	if(enemy.velocity.y < 0) {
		enemy.switchSprite('jump');
	} else if (enemy.velocity.y > 0) {
		enemy.switchSprite('fall');
	}
	
	//Detect collision Player to Enemy Hit
	if	(
		rectangularCollision({
			rectangle1: player,
			rectangle2: enemy
		}) &&
	    player.isAttacking &&
		player.framesCurrent === 4
	)   {
		enemy.takeHit();
		player.isAttacking = false;

		gsap.to('#enemyHealth', {
			width: enemy.health + '%'
		})
	}

	//If Player misses
	if(player.isAttacking && player.framesCurrent === 4) {
		player.isAttacking = false;
	}
	
	//Detect collision Enemy to Player Hit
	if	(
		rectangularCollision({
			rectangle1: enemy,
			rectangle2: player
		}) &&
	    enemy.isAttacking &&
		enemy.framesCurrent === 2
	)   {
		player.takeHit();
		enemy.isAttacking = false;

		gsap.to('#playerHealth', {
			width: player.health + '%'
		})
	}

	//If Enemy misses
	if(enemy.isAttacking && enemy.framesCurrent === 2) {
		enemy.isAttacking = false;
	}

	//End Game Based on Health
	if(enemy.health <= 0 || player.health <= 0) {
		determineWinner({player, enemy, timerId});
	}
}

animate();

//No Input No Game to be Played
window.addEventListener('keydown', (event) => {
	if(!player.dead) {
		switch(event.key) {
			//Player Keys
			case 'd':
				keys.d.pressed = true;
				player.lastKey = 'd';
				break;
			case 'a':
				keys.a.pressed = true;
				player.lastKey = 'a';
				break;
			case 'w':
				player.velocity.y = -17.5;
				break;
			case ' ':
				player.attack();
				break;
		}
	}

	if(!enemy.dead) {
		switch(event.key) {
			//Enemy Keys
			case 'ArrowRight':
				keys.ArrowRight.pressed = true;
				enemy.lastKey = 'ArrowRight';
				break;
			case 'ArrowLeft':
				keys.ArrowLeft.pressed = true;
				enemy.lastKey = 'ArrowLeft';
				break;
			case 'ArrowUp':
				enemy.velocity.y = -17.5;
				break;
			case 'ArrowDown':
				enemy.attack();
				break;
		}
	}
})

//Feedback
window.addEventListener('keyup', (event) => {
	switch(event.key) {
		case 'd':
			keys.d.pressed = false;
			break;
		case 'a':
			keys.a.pressed = false;
			break;
	}
	
	//Enemy Keys
	switch(event.key) {
		case 'ArrowRight':
			keys.ArrowRight.pressed = false;
			break;
		case 'ArrowLeft':
			keys.ArrowLeft.pressed = false;
			break;
	}
})